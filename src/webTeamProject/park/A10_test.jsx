...
function A10_test() {
    // 상태 관리
    const [images, setImages] = useState([null, null, null, null, null]); // 선택된 이미지 배열
    const [selectedButton, setSelectedButton] = useState(null); // 선택된 버튼 인덱스
    const [shuffledImageList, setShuffledImageList] = useState([]); // 섞인 이미지 목록
    const [correctCount, setCorrectCount] = useState(0); // 정답 수

    // 컴포넌트 마운트 시 섞인 이미지 목록 초기화
    useEffect(() => {
